...
class DocLogin extends Component {
  ...
  async checkDoc(event) {
    ...
    var result = null
    try {
      let nid_number = document.getElementById('doc_nid_number').value;
      result = await this.cont['OPT'].methods.checkDoctorInfo(nid_number).call({ from: this.Acc[0] });
      console.log(result);
      if(!result[0])
  ...
  async checkPat(event) {
    ...
    var result = null;
    try {
      let nid_number = document.getElementById('pat_nid_number').value;
      result = await this.cont['OPT'].methods.checkPatientInfo(nid_number).call({ from: this.Acc[0] });
      console.log(result);
      if(!result[0])
        alert('Invalid Credentials. Make sure Account Address and NID Number is entered correctly');
      else
  ...
  render() {
    ...
    const docForm =
      <div className="container">
        ...
        <div style={{ marginLeft: '20px' }}>
          <form>
            <div className="label mt-2"><b>NID Number</b></div>

            <input type="text" name="nid_number" id="doc_nid_number" placeholder="NID Number"></input>
            <br></br>
    ...
    const patForm =
      <div>
        ...
        <div>
          ...
          <input type="text" name="nid_number" id="pat_nid_number" placeholder="NID Number"></input>
          <br></br>
