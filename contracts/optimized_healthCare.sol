...
contract optimized_healthCare {
  ...
  mapping (address => nid) patient_nid_info;
  mapping (address => nid) doctor_nid_info;
  ...
  constructor() public {
          ...
          owner = 0xeaD5Ba267d083077e36f8337E7992d6e3349f624;
  }
  ...
  function signupPatient(string memory _name, string memory _contact, string memory _gender) public {
    ...
    nid memory a = patient_nid_info[msg.sender];
    //Check if the patient already exists by address
    require(!(p.id > address(0x0)));
    //Add patient to blockchain
    require(a.nid_number > 0);

    patients[msg.sender] = patient({name:_name, DOB: a.DOB, id:msg.sender, nid_number: a.nid_number, gender:_gender, contact_info:_contact, files:new bytes32[](0), doctor_list:new address[](0)});
  ...
  function signupDoctor(address _id, string memory _name, string memory _contact, string memory _specialization) public {
      ...
      nid memory a = doctor_nid_info[_id];
      ...
      doctors[_id] = doctor({name:_name, id:_id, contact:_contact, nid_number: a.nid_number, specialization: _specialization, patient_list:new address[](0)});
  ...
  function checkDoctorInfo(uint64 _nid_number) public view returns(bool, address) {
    
    doctor memory d = doctors[msg.sender];
    nid memory a = doctor_nid_info[msg.sender];
    if (a.nid_number == _nid_number){
      return (true, d.id);
    }
  ...
  function checkPatientInfo(uint64 _nid_number) public view returns(bool, address) {
    patient memory p = patients[msg.sender];
    nid memory a = patient_nid_info[msg.sender];
    if (a.nid_number == _nid_number){
      return (true,p.id);
  ...
  function addDoctorNIDInfo(address _doc, string memory _name, string memory _DOB, uint24 _pincode, uint64 _nid_number) public {
    nid memory a = doctor_nid_info[_doc];
    require(a.nid_number == 0);
    doctor_nid_info[_doc] = nid({id:_doc, pincode: _pincode, name: _name, DOB: _DOB, nid_number: _nid_number});
  }

  function addPatientNIDInfo(address _pat, string memory _name, string memory _DOB, uint24 _pincode, uint64 _nid_number) public {
    nid memory a = patient_nid_info[_pat];
    require(a.nid_number == 0);
    patient_nid_info[_pat] = nid({id:_pat, pincode: _pincode, name: _name, DOB: _DOB, nid_number: _nid_number});
  }
